...
// Arduino-ESP32 2.0.17 exposes microsecondsToClockCycles() through a runtime
// CPU-frequency call, which cannot be used in the constexpr timing table.
// ESP-HACK runs on the fixed 240 MHz ESP32 target.
#if defined(ESP32)
constexpr timeOW_t ONEWIREHUB_CYCLES_PER_US = 240;
#else
constexpr timeOW_t ONEWIREHUB_CYCLES_PER_US = microsecondsToClockCycles(1);
#endif

constexpr timeOW_t operator "" _us(const unsigned long long int time_us) // user defined literal used in config
{
    return timeOW_t(time_us * ONEWIREHUB_CYCLES_PER_US / VALUE_IPL); // note: cycles per microsecond == speed in MHz....
...
{
    return (time_us * ONEWIREHUB_CYCLES_PER_US / VALUE_IPL); // note: cycles per microsecond == speed in MHz....
}
