...
extern bool ensureSDReadyInteractive(bool allowSkip);
extern void showSDInitErrorScreen();
extern void invalidateSDCard();
extern bool wasSDInitDismissed();
...
static const unsigned char PROGMEM image_delete_bits[] = {
  
};
...
void ExplorerDrawDeleteConfirm(const ExplorerState& state, DisplayType& display) {
  ...
  explorerPrintMarqueeText(display, state.list[state.index].name, 39, 3, true,
                            marqueeText, marqueeStartedAt, 14);
  display.setCursor(8, 54);
  display.print(F("Press OK to delete."));
  display.drawBitmap(26, 16, image_delete_bits, 76, 32, SH110X_WHITE);
  display.display();
}
...
ExplorerAction ExplorerHandle(ExplorerState& state, const ExplorerConfig& cfg, DisplayType& display,
  ...
  if (state.sdError) {
    // Keep the SD recovery screen active. Back must not exit this screen.
    if (ensureSDReadyInteractive(true)) {
      ExplorerLoad(state, cfg);
      ExplorerDraw(state, display);
      return state.sdError ? EXPLORER_SD_ERROR : EXPLORER_NONE;
    }
    if (wasSDInitDismissed()) return EXPLORER_EXIT;
    showSDInitErrorScreen();
    return EXPLORER_SD_ERROR;
  }

  const bool needsSD = okClick || backHold ||
                       (backClick && cfg.rootDir && state.currentDir != cfg.rootDir);
  if (needsSD && !ensureSDReadyInteractive(true)) {
    state.inDeleteConfirm = false;
    state.sdError = true;
    if (wasSDInitDismissed()) return EXPLORER_EXIT;
    showSDInitErrorScreen();
  ...
  if (state.sdError) {
    if (backClick) return EXPLORER_EXIT;
    if (okClick) {
      ExplorerLoad(state, cfg);
      ExplorerDraw(state, display);
    }
  ...
  if (state.inDeleteConfirm) {
    ...
    static unsigned long lastDeleteConfirmDrawAt = 0;
    if (state.count > 0 && state.list[state.index].name.length() > 14 &&
        millis() - lastDeleteConfirmDrawAt >= 200) {
      lastDeleteConfirmDrawAt = millis();
